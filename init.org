...
my/install-packages= (see the *Elpa* section below). At this point
...
=M-x my/install-packages= to install any missing packages. Note that
...
my/fix-frame= after opening a new frame attached to a running emacs
server process.
