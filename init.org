...
=.dir-locals.el= file high up in the files system could not be found
...
provides contextual menus for =dired=, register keys in =C-x r=, and
the Isearch keys in =M-s=.
