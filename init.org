...
=server-visit-hook= is invoked when emacsclient is called with a file
as an argument; unfortunately, this hook is not run when emacsclient
is not provided a file name argument, in which case fix-frame must be
invoked manually.
