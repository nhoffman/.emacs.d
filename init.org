...
The above functions attempt to use the virtualenv that we'll create in
=~/.emacs.d= (see below) so that, for example, =elpy= can find its
python dependencies.
...
section below for a list of packages installed by this
configuration. First, launch Emacs; I'd recommend launching without
emacs-desktop, for example =emacs -nw --no-desktop=. From within
Emacs, fetch the list of packages with =M-x
package-list-packages=. Finally, install the specified packages with
=M-x package-install-my-packages=. At this point it's usually a good
idea to quit and relaunch Emacs.
...
script, you'll need to initialize the git submodule containing the
=org-export= project (https://github.com/nhoffman/org-export):
...
** Quick and dirty installation of all python dependencies

#+BEGIN_SRC sh :eval no
pip install -U -r \
<(curl --silent https://raw.githubusercontent.com/jorgenschaefer/elpy/master/requirements.txt && \
...
Installation (not on ELPA, alas):

#+BEGIN_SRC sh :eval no :exports code
wget -q https://raw.githubusercontent.com/winterTTr/ace-jump-mode/master/ace-jump-mode.el -O ~/.emacs.d/ace-jump-mode.el
#+END_SRC
