...
The code block below defines the key sequence =C-x /= as a prefix for
the keymap =my-key-map=, then assigns a series of functions to single
characters following this prefix. Use "C-x / /" to list all of the
bindings.
