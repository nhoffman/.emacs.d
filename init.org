...
Here's how publication of the rendered version of this site to GitHub's gh-pages is set up.
...
grep -A 8 publish_log, SConstruct
#+END_SRC
...
#+BEGIN_SRC elisp :tangle init.el
  ...
  ;; (defun package-install-if-missing (pkg)
  ;;   (interactive)
  ;;   (unless (package-installed-not-builtin-p pkg)
  ;;     (cond ((yes-or-no-p (format "Install %s? " pkg))
  ;;            (package-install pkg)))))
  ...
  ;;   ;; Interactively installs packages listed in global 'package-my-package-list'
  ;;   (interactive)
  ;;   (mapcar 'package-install-if-missing package-my-package-list))
#+END_SRC
