...
A quick cursor location minor mode for emacs. I picked =M-C-'= as the
key combo for activation.

Installation (not on ELPA, alas):

#+BEGIN_SRC sh :eval no :exports code
wget -q \ https://raw.githubusercontent.com/winterTTr/ace-jump-mode/master/ace-jump-mode.el \
-O ~/.emacs.d/ace-jump-mode.el
#+END_SRC
